...
class RotaryProjection(Projection):
    def __init__(self, *, proj_width: int, num_heads: int, base: int = 10000):
...
class QueryKeyProjection(nn.Module):
    def __init__(self, dim: int, num_heads: int, proj_layer, kwargs=None, partial_factor=None, pos_embed_type="rope"):
        super().__init__()
    ...
    def forward(self, query, key, query_id, kv_id):
        '''
        Input:
            query: [B, total_len, d_model]
            key: [B, total_len, d_model]
            query_id: [B, total_len]
            kv_id: [B, total_len]
        '''
